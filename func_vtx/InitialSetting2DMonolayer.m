...
elseif INIT_VER==2
    ...
    tie.numb=initprm.cnumb*2;
elseif INIT_VER==3 || INIT_VER==4 || INIT_VER==5
    cell.numb=initprm.cnumb;
    vtx.numb=(initprm.cnumb+1)*2;
    edge.numb=initprm.cnumb*3+1;
    tie.numb=initprm.cnumb*2;
...
vtx.f_cellcurv=zeros(vtx.numb,2);
...
if INIT_VER==3 || INIT_VER==4 || INIT_VER==5
    cell.div(1)=0; cell.div(2)=0; cell.div(cell.numb-1)=0; cell.div(cell.numb)=0;
end
...
elseif INIT_VER==2 || INIT_VER==3 || INIT_VER==4 || INIT_VER==5
    cell.cell(1,1)=0;
...
if INIT_VER==2 || INIT_VER==3 || INIT_VER==4 || INIT_VER==5
...
els1=find(edge.type==1); % aplical
els2=find(edge.type==2); % lateral
els3=find(edge.type==3); % basal

edge.original_length_apical=mean(edge.dist(els1)) * 1.0;
edge.original_length_lateral=mean(edge.dist(els2)) * 1.0;
edge.original_length_basal=mean(edge.dist(els3)) * 1.0;

edge.target_dist(els1,1)=edge.original_length_apical;
edge.target_dist(els2,1)=edge.original_length_lateral;
edge.target_dist(els3,1)=edge.original_length_basal;
%edge.target_dist(els3,1)=edge.original_length_apical;
...
%% F-actin - 210305 add
cell.Factin=zeros(cell.numb,1);

vtx.n_norm_vec=zeros(vtx.numb,2);
