...
% noise
ns1 = 0.001*(rand(1,2)-0.5);
ns2 = 0.001*(rand(1,2)-0.5);
...
edge.target_dist(edge.numb+1)=edge.original_length_apical;
edge.target_dist(edge.numb+2)=edge.original_length_lateral;
edge.target_dist(edge.numb+3)=edge.original_length_basal;
...
% 210305 add
cell.Factin(cell.numb+1) = cell.Factin(cid);
...
vtx.f_cellcurv(vtx.numb+1:vtx.numb+2,1:2)=zeros(2,2);

vtx.f_luminal(vtx.numb+1:vtx.numb+2,1:2)=zeros(2,2);
vtx.f_outward(vtx.numb+1:vtx.numb+2,1:2)=zeros(2,2);
vtx.f_tie(vtx.numb+1:vtx.numb+2,1:2)=zeros(2,2);
vtx.f_hitavoid(vtx.numb+1:vtx.numb+2,1:2)=zeros(2,2);
